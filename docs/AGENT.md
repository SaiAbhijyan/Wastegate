...
- The full harness skill (~16.6 KB) is sent only if the whole system prompt fits `[agent] max_system_bytes` (default 12000). Otherwise the model gets a short code-gate note (`harness=code-only`); the code-enforced VERIFICATION gate is the same either way. Per-call `max_tokens` is capped per tier (cheap 4096, mid 8192). OpenAI-compatible adapters (Groq, Gemini, OpenRouter, Ollama) declare the tools as native function tools (`read`, `grep`, `edit`, `pytest`, `shell`). The loop accepts native `tool_calls` and the text protocol. Invented names are mapped (`repo_browser.grep` → `grep`) or answered with `unknown tool`. A Groq `tool_use_failed` 400 with a parseable `failed_generation` is executed and the loop continues (at most 2 per loop). Live Groq runs, N=1 each, wiring only: `results/20260925-live-agent.md` got a 400 before tools were declared. `results/20260925-live-agent-2.md` ran read → edit → pytest exit 0 (VERIFICATION: PASS), then got a 400 when the model called an undeclared `DONE` tool with non-JSON arguments, so the run exited 1. Finishing the loop live is not yet shown.
